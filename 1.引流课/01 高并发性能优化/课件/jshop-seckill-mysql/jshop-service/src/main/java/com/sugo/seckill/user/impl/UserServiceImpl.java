...
public class UserServiceImpl implements UserService {
    ...
    public HttpResult login(FrontUser frontUser) {
        ...
        //把用户信息存储在Redis
        redisTemplate.opsForValue().set(token, JsonUtils.objectToJson(tbFrontUser),1, TimeUnit.HOURS);
    ...
    public FrontUser getUserInfoFromCache(String token) {
        ...
        if(StringUtils.isNotBlank(userJson)){
            FrontUser frontUser = JsonUtils.jsonToPojo(userJson, FrontUser.class);
            return frontUser;
        }
        return null;
    }
