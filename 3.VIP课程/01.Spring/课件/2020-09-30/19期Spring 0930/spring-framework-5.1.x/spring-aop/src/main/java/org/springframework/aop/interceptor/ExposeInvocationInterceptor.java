...
public final class ExposeInvocationInterceptor implements MethodInterceptor, PriorityOrdered, Serializable {
	...
	//它是ThreadLocal同一线共享数据，要把MethodInvocation要共享，然后我们的MethodInvocation就是我们上面的 mi=CglibAopProxy,进行共享
	...
	public Object invoke(MethodInvocation mi) throws Throwable {
		MethodInvocation oldInvocation = invocation.get();//什么是invocation
		invocation.set(mi);//第一次mi还没共享，所以先设置进去
		try {
			//重新进行CglibProxy.proceed(),执行第二个拦截器 （interceptIndex已自增了，所以为第二个拦截器）
			return mi.proceed();
		}
		finally {
			//点击进入invocation
