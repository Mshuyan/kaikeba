...
+ 运行时常量池
  ...
  class文件加载到内存之后，之前静态常量池除了字符串字面量之外的其他内容在内存中加载成了运行时常量池

  每个Class对象有1个运行时常量池

+ 字符串常量池

  静态常量池中所有字符串字面量加载到内存后，统一汇总到了字符串常量池
...
+ 在字符串常量池中会存储1个`stringtable`，其数据结构与`hashtable`完全一样，用来加快查找效率
+ `key`是字符串的hash值，`value`是字符串字面量的实际内存地址

+ 1.6以前是哈希桶长度固定`1009`，1.7以后可以通过`-XX:StringTableSize=<size>`进行设置
+ 常量存入stringtable过程
  + 类加载时字符串常量已经加载进字符串常量池
  + **第一次使用该常量时**，先在堆中创建这个常量对应的对象
...
![image-20201023043612547](assets/image-20201023043612547.png) 
