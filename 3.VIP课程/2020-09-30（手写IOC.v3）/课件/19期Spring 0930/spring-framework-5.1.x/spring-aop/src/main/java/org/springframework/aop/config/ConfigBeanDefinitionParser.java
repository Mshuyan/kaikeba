...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
		...
		try {
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (isAdviceNode(node, parserContext)) {
					...
					// 一个<aop:before>标签就对应一个Advisor，一个Advisor对应一个Advice
	...
	private AbstractBeanDefinition parseAdvice(
		...
		try {
			...
			// MethodLocatingFactoryBean封装了通知功能对应的方法名称
			// 在该类中可以对字符串的method方法转成Method对象
			...
			// SimpleBeanFactoryAwareAspectInstanceFactory里面含有BeanFactory的引用
			// 可以通过Bean的名称，去BeanFactory中获取对象
			RootBeanDefinition aspectFactoryDef =
					new RootBeanDefinition(SimpleBeanFactoryAwareAspectInstanceFactory.class);
			// aspectName就是通知功能对应的Bean实例的名称
